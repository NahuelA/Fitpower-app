 ...
 * @author NahuelA
 */
public class TipoUsuarioRepository implements Serializable {

    public TipoUsuarioRepository(EntityManagerFactory emf) {
        this.emf = emf;
    }
    ...
    public TipoUsuario findTipoUsuarioByName(String nombre){
        EntityManager em = getEntityManager();

        String jpql = "SELECT u FROM TipoUsuario u WHERE u.nombre = :nombre";
        TypedQuery<TipoUsuario> query = em.createQuery(jpql, TipoUsuario.class);
        query.setParameter("nombre", nombre);
        query.setMaxResults(1);

        List<TipoUsuario> resultados = query.getResultList();
